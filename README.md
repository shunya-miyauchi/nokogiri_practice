...
文字列は、内部的には常にUTF-8で保存されます。テキスト値を返すメソッドは常にUTF-8でエンコードされた文字列を返します。マークアップを含む文字列を返すメソッド (to_xml, to_html, inner_html など) は、ソースドキュメントと同じようにエンコードされた文字列を返します。
...
文書によっては、あるエンコーディングを宣言しておきながら、実際には別のエンコーディングを使用しているものがあります。このような場合、パーサーはどちらのエンコードを選択すべきでしょうか。
...
データは単なるバイトの流れです。人間はそのストリームに意味を付加します。libxml2 は最善を尽くしますが、常に正しいというわけではありません。

If you want Nokogiri to handle the document encoding properly, your
best bet is to explicitly set the encoding.  Here is an example of
explicitly setting the encoding to EUC-JP on the parser:
Nokogiriにドキュメントのエンコーディングを正しく扱わせたいなら、最善の方法は、明示的にエンコーディングを設定することです。以下はパーサー上で明示的にエンコーディングを EUC-JP に設定する例です。
